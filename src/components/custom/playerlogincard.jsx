...
const Playerlogincard = () => {
  ...
  const handlePlayerLogin = async () => {
    ...
    try {
      ...
      if (!responseData.success) {
        ...
        dispatch(
          Login({
            uid: responseData.message.uid,
            name: responseData.message.name,
            game: responseData.message.game,
          })
        );
      }
    } catch (error) {
  ...
  const deletePlayerFromList = (uidToDelete) => {
    ...
    setPrevPlayers((prev) =>
      prev.filter((player) => player.uid !== uidToDelete)
    );
  };
  ...
  return (
    ...
    >
      <div className="flex items-center justify-end gap-2 max-[736px]:w-full">
        {user?.loggedIn && (
          <div
            className={`flex items-end max-[736px]:w-full justify-center gap-28 hover:cursor-pointer ${
              user ? "max-[736px]:justify-between" : "max-[736px]:items-start"
            }`}
          >
            <div
        ...
        {!user.loggedIn && (
          <div
            className={`flex items-end justify-center gap-28 hover:cursor-pointer ${
              user ? "max-[736px]:justify-between" : "max-[736px]:items-start"
            }`}
          >
            <p className="text-xl font-bold font-ar ">
      ...
      {!user?.loggedIn ? (
        <div className="w-fit max-[736px]:w-full h-fit bg-[#eee] rounded-lg mt-5 gap-5 border-[#e5e7eb] border flex flex-col items-end py-4 pb-6 px-8 justify-center">
          <div className="flex flex-col items-end justify-center gap-2 max-[736px]:w-full">
            ...
            <div
              dir={language === "en" ? "rtl" : "ltr"}
              className="w-full flex max-[736px]:w-full"
            >
              <Button
                disabled={loading}
                style={{
                  borderTopRightRadius: language === "en" && "8px",
                  borderBottomRightRadius: language === "en" && "8px",
                  borderTopLeftRadius: language === "en" && "0px",
                  borderBottomLeftRadius: language === "en" && "0px",
                }}
              ...
              <div className="relative max-[736px]:w-full">
                <Input
                  ...
                  value={uid}
                  style={{
                    borderTopRightRadius: language === "en" && "0",
                    borderBottomRightRadius: language === "en" && "0px",
                    borderTopLeftRadius: language === "en" && "8px",
                    borderBottomLeftRadius: language === "en" && "8px",
                  }}
                  onChange={(e) => setUid(e.target.value)}
                  className={`font-ar relative w-[400px] max-[736px]:w-full ${
                    language === "ar" ? "pr-12" : "pl-12"
                  } rounded-tl-none rounded-bl-none bg-white`}
                  placeholder={
                    language === "en"
                      ? "Please enter player ID here"
                      : "يرجى إدخال معرف اللاعب هنا"
                  }
                />
                <svg
                  className="ssss"
            ...
            <div
              dir={language === "en" ? "ltr" : "rtl"}
              style={{ width: "100%" }}
              className="relative flex flex-col items-center gap-4 text-xs/normal text-text-secondary md:text-sm/[22px]"
            >
